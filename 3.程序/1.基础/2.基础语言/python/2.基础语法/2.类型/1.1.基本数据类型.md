...
使用`type(原值)`格式进行强转，例如：`int(222.22)`转换后为`222`
...
- Python 3中只有`int`一种整数类型，支持任意精度
- 浮点数遵循IEEE 754标准，可能存在精度问题
- 布尔值可以参与数值运算：`True`为1，`False`为0
...
text = 'Runoob'  # 定义一个字符串变量（避免使用str作为变量名）

print(text)           # 打印整个字符串
print(text[0:-1])     # 打印字符串第一个到倒数第二个字符（不包含倒数第一个字符）
print(text[0])        # 打印字符串的第一个字符
print(text[2:5])      # 打印字符串第三到第五个字符（包含第五个字符）
print(text[2:])       # 打印字符串从第三个字符开始到末尾
print(text * 2)       # 打印字符串两次
print(text + "TEST")  # 打印字符串和"TEST"拼接在一起
...
4. Python中的字符串是**不可变**的数据类型，一旦创建就不能修改。

#### f-string（字面量格式化字符串）

Python 3.6 之后版本添加的，称之为字面量格式化字符串，是新的格式化字符串的语法。
> 类似kotlin里面的`${}`
...
23. **`zfill(width)`**：用零填充字符串，使其长度达到 `width`。

24. **`center(width[, fillchar])`**：返回一个原字符串居中的字符串，总长度为 `width`，如果需要的话，会在两侧填充 `fillchar` 字符。

25. **`ljust(width[, fillchar])`**：返回一个原字符串左对齐的字符串，总长度为 `width`，如果需要的话，会在右侧填充 `fillchar` 字符。

26. **`rjust(width[, fillchar])`**：返回一个原字符串右对齐的字符串，总长度为 `width`，如果需要的话，会在左侧填充 `fillchar` 字符。
...
- 以下值被认为是 `False`：`None`, `False`, 数值0, 空序列/集合(`""`, `[]`, `{}`, `set()`)
- 其他所有值都被认为是 `True`
...
`None` 是Python中的特殊常量，表示"无"或"空"的概念。
...
- `None` 是单例对象，应该使用 `is` 而不是 `==` 来比较
- 函数如果没有显式返回值，默认返回 `None`
- 在类型注解中，使用 `Optional[T]` 或 `Union[T, None]` 表示可能为None的类型

---

### List

列表是Python中最常用的数据结构之一，类似于其他语言中的动态数组。
...
# 创建列表
my_list = []                    # 空列表
my_list = list()                # 函数定义空列表
numbers = [1, 2, 3, 4, 5]      # 数字列表
mixed = ['abcd', 786, 2.23, 'runoob', 70.2]  # 混合类型列表
tiny_list = [123, 'runoob']

print(mixed)            # 打印整个列表
print(mixed[0])         # 打印列表的第一个元素
print(mixed[1:3])       # 打印列表第二到第三个元素（不包含第四个元素）
print(mixed[2:])        # 打印列表从第三个元素开始到末尾
print(tiny_list * 2)    # 打印tiny_list列表两次
print(mixed + tiny_list)  # 打印两个列表拼接在一起的结果
...
元组（tuple）与列表类似，不同之处在于元组的元素不能修改（不可变），但可以包含可变的对象。元组写在小括号 () 里，元素之间用逗号隔开。

```python
#!/usr/bin/python3

my_tuple = ('abcd', 786, 2.23, 'runoob', 70.2)  # 定义元组
...
2. 元组与字符串类似，可以被索引且下标索引从0开始，-1 为从末尾开始的位置。也可以进行截取。
3. 元组是**不可变**的，一旦创建就不能修改其内容
4. 元组可以作为字典的键（因为不可变性）
...
`frozenset` 是 `set` 的不可变版本，一旦创建就不能修改。由于其不可变性，`frozenset` 可以作为字典的键或其他集合的元素。
...
# frozenset支持大部分set的操作（除了修改操作）
print(len(fs))        # 5
print(3 in fs)        # True
print(fs | {6, 7})    # frozenset({1, 2, 3, 4, 5, 6, 7})

# frozenset可以作为字典的键
dict_with_frozen = {fs: "immutable set"}
print(dict_with_frozen)

# 不能修改frozenset
# fs.add(6)  # 这会抛出AttributeError
```
...
字典是一种映射类型，字典用 `{ }` 标识，它是一个无序的 **键(key) : 值(value)** 的集合。**键(key)**必须使用不可变类型（如字符串、数字、元组），且必须唯一。

```python
#!/usr/bin/python3

my_dict = {}  # 空字典（避免使用dict作为变量名）
my_dict['one'] = "1 - 菜鸟教程"
my_dict[2] = "2 - 菜鸟工具"
...
8. **`popitem()`**：移除并返回字典中的最后一个插入的键值对（Python 3.7+保证是最后插入的），如果字典为空则抛出 `KeyError`。
...
13. **`len(d)`**：返回字典 `d` 中键值对的数量。

14. **字典推导式**：可以使用 `{key: value for (key, value) in iterable}` 形式，快速生成字典。

15. **`defaultdict`**：`collections` 模块中的 `defaultdict` 类是字典的一个子类，它可以在访问不存在的键时提供一个默认值。

**现代字典特性（Python 3.7+）**：

- 字典保持插入顺序（有序字典）
- 内存效率更高
- 支持合并操作符（Python 3.9+）：`dict1 | dict2`
...
`bytes` 是一个不可变的序列类型，用于表示字节数据（0-255之间的整数序列）。
...
# bytes与字符串的转换
text = "你好"
encoded = text.encode('utf-8')  # 字符串 -> bytes
decoded = encoded.decode('utf-8')  # bytes -> 字符串
...
Python中的序列类型（如str、list、tuple、bytes）都支持以下通用操作：

- **索引访问**：`seq[index]`
- **切片**：`seq[start:end:step]` 特别说明：`[::-1]` 表示反转的切片操作
    >start：起始索引（包含）,stop：结束索引（不包含）,step：步长（可正可负）
    >省略 start：默认从序列开头开始（索引 0 或 -1，取决于 step 方向）,省略 stop：默认到序列末尾结束,省略 step：默认步长为 1
    >示例：[7::] 指从7开始到结尾的列表数据
- **长度**：`len(seq)`
- **成员测试**：`item in seq`
- **连接**：`seq1 + seq2`（对于相同类型）
...
maybe_value: Optional[str] = None  # 等价于 Union[str, None]
```
