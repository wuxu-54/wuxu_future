...
* **StatefulWidget**：这是一个拥有自己状态的widget。它有一个关联的State对象，该对象包含了widget的状态数据和逻辑。
  ...
  * **didChangeDependencies**：在initState之后，每次widget的依赖项发生变化时（例如，当它的BuildContext发生变化，或者其ancestor的State发生变化时），都会调用didChangeDependencies。这在initState之后至少会被调用一次。
    * 触发 didChangeDependencies 的唯一“硬性条件”：
      * 首次挂载（initState 之后强制调用一次）。
      * 父级 InheritedWidget 发生了改变，并且该父级在 updateShouldNotify 中返回了 true，且当前 State 通过 dependOnInheritedWidgetOfExactType 注册了依赖。
...
## didUpdateWidget 与 didChangeDependencies 区别

* 核心区别：触发源头不同

  * didUpdateWidget：当父 Widget 重新构建，并且传给了当前 Widget 新的参数（widget 属性）时触发。它是“自上而下”的传递。

  * didChangeDependencies：当该 State 对象所依赖的 InheritedWidget 发生变化时触发（例如 Theme、MediaQuery、Locale、Provider 更新）。它是“自外向内”的注入，或者在 initState 之后首次构建前一定会被调用一次。

* 参数与可见性

  * didUpdateWidget：带有一个参数 covariant T oldWidget，你可以通过 oldWidget 和当前的 widget 进行对比，精准判断到底哪个属性变了（例如：if (widget.id != oldWidget.id)），从而决定是否需要执行昂贵的重计算。

  * didChangeDependencies：没有参数。你无法知道具体是哪个依赖变了，只能知道“有依赖变了”。通常配合 context.dependOnInheritedWidgetOfExactType 使用。

* 首次调用时机

  * didUpdateWidget：不会在 initState 之后立即调用。只有当父组件发生重建并传入新实例时才会调用。

  * didChangeDependencies：一定会在 initState 之后立即调用一次，之后每当依赖变化时再次调用
