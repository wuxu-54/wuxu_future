...
* Middleware：中间件函数，可以过滤处理 Store分发的数据，比如进行登录状态判断、网络接口请求等。
...
结合之前学的 Consumer、Selector，可以直接一一对应：

| Redux 体系 | Provider 体系 | 核心作用 |
| --- | --- | --- |
| `StoreProvider` | `ChangeNotifierProvider` | 在 Widget 树中注入状态仓库 |
| `StoreConnector` | `Selector` | 筛选状态、转换数据、精准局部刷新 |
| `StoreBuilder` | `Consumer` | 全量监听状态、拿到完整状态实例 |
| `ViewModel` | 筛选后的数据 | UI 层直接使用的业务数据 |

一句话总结：**StoreConnector 就是 Redux 版本的 Selector，二者是同一种设计思想在不同状态框架下的实现。**
