...
可以在Application中添加如下代码：

```kotlin
if (BuildConfig.DEBUG) {
    var messageRunTime: Long = 0
    Looper.getMainLooper().setMessageLogging { s ->
        if (s.startsWith(">>>>> Dispatching to")) {
            // 消息开始分发时记录开始时间
            messageRunTime = System.currentTimeMillis()
        } else if (s.startsWith("<<<<< Finished to")) {
            // 消息处理完成时计算处理耗时
            val runTime = System.currentTimeMillis() - messageRunTime
            messageRunTime = System.currentTimeMillis()
            if (runTime > 2000) {
                Log.w(
                    "LooperMonitor",
                    "runTime too Long,time is $runTime,s is : $s"
                )
            }
        }
    }
